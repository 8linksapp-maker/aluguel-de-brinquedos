...
import festa from '../data/themes/festa.json';
...
export const PRESETS: Record<string, ThemePreset> = { dental, harmonizacao, vidro, festa };
...
export function resolveTheme(): { vars: Record<string, string>; preset: ThemePreset } {
  const preset = PRESETS[themeConfig.preset] ?? PRESETS.festa;
